export class DelDNSModule {
  static async render(container, state) {
      ...
      <div class="page-header d-print-none mb-3">
        <div class="row align-items-center">
          <div class="col">
            ...
            <h2 class="page-title fw-bold">批量删除解析记录 (Batch Delete DNS)</h2>
          </div>
        </div>
      </div>
          ...
          <div class="card border-0 shadow-sm rounded-3 h-100">
            <div class="card-body">
              ...
              <div class="mb-3">
                ...
                <select id="del-account" class="form-select border-2 shadow-none">
              ...
              <div class="mb-3">
                ...
                <textarea id="del-domains" class="form-control border-2 shadow-none font-monospace" rows="8" placeholder="example.com\nexample.net\nexample.org"></textarea>
              </div>
              <div class="mb-3">
                <label class="form-label fw-bold">记录类型</label>
                <select id="del-record-type" class="form-select border-2 shadow-none">
              ...
              <div class="mb-3">
                ...
                <input type="text" id="del-host-record" class="form-control border-2 shadow-none" placeholder="留空表示全部，如：www、@、*">
              </div>
              <div class="mb-3">
                <label class="form-label fw-bold">删除选项</label>
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="del-all">
                  <label class="form-check-label" for="del-all">
                    删除所有记录
                  </label>
                </div>
              </div>
              <div class="alert alert-danger border-0 py-2 px-3">
                ...
                <span class="small fw-bold">警告：删除DNS记录后无法恢复，请谨慎操作！</span>
              </div>
              <div class="form-footer mt-4">
                <button id="btn-start-delete" class="btn btn-danger w-100 py-2 fw-bold shadow-sm">
        ...
        <div class="col-md-7">
          <div class="card border-0 shadow-sm rounded-3 h-100">
            <div class="card-body">
              ...
              <div id="del-results" class="table-responsive mt-2">
    ...
    document.getElementById('btn-start-delete').addEventListener('click', () => this.startDelete(state));
  }

  static async startDelete(state) {
    const accountId = document.getElementById('del-account').value;
    const domainsText = document.getElementById('del-domains').value || '';
    const recordType = document.getElementById('del-record-type').value;
    const hostRecord = document.getElementById('del-host-record').value.trim();
    const deleteAll = document.getElementById('del-all').checked;
    ...
    if (!confirm(`确定要删除 ${domains.length} 个域名的DNS记录吗？此操作不可撤销！`)) return;

    const btn = document.getElementById('btn-start-delete');
    const resultsDiv = document.getElementById('del-results');
    ...
    try {
      const res = await fetch('/api/dns/batch-delete', {
        method: 'POST',
        headers: { 
